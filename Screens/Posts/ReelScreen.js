...
const ReelScreen = () => {
  ...
  const { tReelScreen, t } = useTranslation();
  ...
  const {
    ...
    isLiked = false,
    likesCount = 0,
    isFollowed = false, // Add follow state from route params
  } = route.params || {};
  ...
  const [followed, setFollowed] = useState(isFollowed); // Add follow state
  ...
  // Helper function to determine video aspect ratio for ReelScreen
  ...
  const handleFollowToggle = () => {
    ...
    console.log('Follow/Unfollow pressed for user:', user.id || 'unknown');
  };
  ...
  const handleTap = () => {
    if (isLoading || videoError) {
      return; // 🔒 Don't allow play/pause or like while loading or error
    }
    ...
    <View style={styles.container}>
      ...
      <TouchableWithoutFeedback onPress={handleTap}>
        <View style={styles.videoContainer}>
          ...
          <Video
            ...
            style={[
              styles.video,
              // Apply styling based on video aspect ratio for ReelScreen
              (() => {
      ...
      <View style={styles.rightIcons}>
        <TouchableOpacity style={styles.iconButton} onPress={handleLike}>
          <Ionicons name={liked ? 'heart' : 'heart-outline'} size={32} color={liked ? '#ff3040' : '#fff'} />
        ...
        <View style={styles.userRow}>
          ...
          <View style={styles.userInfo}>
            <View style={styles.nameRow}>
              ...
              {/* Follow button next to the name - only show if not followed */}
              {!followed && (
                <TouchableOpacity
                  style={[styles.followButton, { backgroundColor: theme.primary }]}
                  onPress={handleFollowToggle}
                >
                  <Text style={styles.followButtonText}>{tReelScreen('follow')}</Text>
                </TouchableOpacity>
              )}
            </View>
            <Text style={styles.timestamp}>{formatTimestamp(timestamp, tReelScreen)}</Text>
          </View>
        </View>
      ...
      >
        ...
        >
          <View style={[styles.modalContainer, { backgroundColor: theme.background }]}>
              ...
              <>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    handleFollowToggle(); // This will unfollow
                  }}
                >
                  <Ionicons name="person-remove-outline" size={20} color="#262626" style={styles.optionIcon} />
                  <Text style={[styles.optionText, { color: theme.textPrimary }]}>
                    {console.log('Debug - firstName:', firstName, 'lastName:', lastName, 'user:', user)}
                    {`${tReelScreen('unfollow').replace('{firstName}', firstName || 'User').replace('{lastName}', lastName || '')}`}
                  </Text>
                </TouchableOpacity>
            ...
            >
              <Ionicons name="flag-outline" size={20} color="#ed4956" style={styles.optionIcon} />
                  <Text style={[styles.optionText, styles.reportText, { color: theme.error }]}>
                    {`${tReelScreen('report').replace('{firstName}', firstName || 'User').replace('{lastName}', lastName || '')}`}
                  </Text>
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // Video aspect ratio styles for ReelScreen
  verticalVideo: {
